...
def get_or_create(conn, table: str, match: dict, insert: dict) -> int:
    ...
    result = conn.execute(
        text(f"INSERT INTO {table} ({cols}) VALUES ({vals}) RETURNING id"), insert
    )
...
def load_dim_date(conn, d: date) -> int:
    return get_or_create(
        conn, "dim_date",
        match={"date": d},
        insert={"date": d, "year": d.year, "month": d.month, "day": d.day, "weekday": d.weekday()},
    )
...
def run(trusted_dir: str = "data/trusted", db_url: str = None) -> dict:
    ...
    try:
        dataset = pq.read_table(trusted_dir)
        df_all = dataset.to_pandas()
        log.info("Lidos %d registros (partições incluídas)", len(df_all))
    except Exception as e:
        log.error("Erro ao ler dataset: %s", e)
        return {"loaded": 0, "skipped": 0}

    if df_all.empty:
        log.warning("Dataset vazio em %s", trusted_dir)
        return {"loaded": 0, "skipped": 0}
    ...
    with engine.begin() as conn:
        for _, row in df_all.iterrows():
            try:
                ...
                country_code = str(row.get("country") or "XX").strip().upper()[:10]
                location_id = load_dim_location(conn, country_code, row.get("city") or row.get("location"))
